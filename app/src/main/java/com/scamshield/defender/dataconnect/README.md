...
with service ID `scamshield-fdc`
...
import com.scamshield.defender.dataconnect.*
```
...
is the query named "GetBlockedNumbers",
which can be accessed via the [DefaultConnector.getBlockedNumbers] property.


An example of the property for a mutation
is the mutation named "AddCallLog",
which can be accessed via the [DefaultConnector.addCallLog] property.
...
For example, the "GetUserProfile" query has no variables
and can be executed via the
[DefaultConnector.getUserProfile]
property as follows:

```kotlin
val connector = DefaultConnector.instance
val queryResult = connector.getUserProfile.execute()
println("GetUserProfile query returned: ${queryResult.data}")
```
...
For example, the "GetBlockedNumbers" query has 1 required variable ("blockedBy")
and can be executed via the [DefaultConnector.getBlockedNumbers]
property as follows:

```kotlin
val connector = DefaultConnector.instance
val queryResult = connector.getBlockedNumbers.execute(blockedBy="garply")
println("GetBlockedNumbers query returned: ${queryResult.data}")
```
...
For example, the "AddCallLog" mutation has 5 required variables ("phoneNumber", "callDurationSeconds", "scamScore", "isScam", and "deepfakeScore")
and can be executed via the [DefaultConnector.addCallLog]
property as follows:

```kotlin
val connector = DefaultConnector.instance
val mutationResult = connector.addCallLog.execute(phoneNumber="thud", callDurationSeconds=9736, scamScore=1880.85, isScam=false, deepfakeScore=1143.59)
println("AddCallLog mutation returned: ${mutationResult.data}")
```
...
For example, the "UpsertUserProfile" mutation has 3 optional variables ("phoneNumber", "email", and "country")
and can be executed via the [DefaultConnector.upsertUserProfile]
property as follows:
